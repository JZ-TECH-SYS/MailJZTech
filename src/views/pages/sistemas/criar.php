...
document.getElementById('formCriarSistema').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!data.error && data.result) {
            // Mostrar SweetAlert2 com chave de API
            Swal.fire({
                icon: 'success',
                ...
                html: `
                    ...
                    <div class="mb-3 text-start">
                        ...
                        <p>${data.result.nome || formData.nome}</p>
                    </div>
                    <div class="mb-3 text-start">
                        <label class="form-label"><strong>Chave de API:</strong></label>
                        <div class="input-group">
                            <input type="text" class="form-control" id="chaveApi" value="${data.result.chave_api}" readonly>
                            <button class="btn btn-primary" onclick="copiarChaveSweetAlert()">
                ...
                confirmButtonText: '<i class="fas fa-list"></i> Ir para Sistemas',
                preConfirm: () => {
                    window.location.href = '<?php echo $base; ?>/sistemas';
                }
            });
        } else {
            mostrarErro('Erro: ' + (data.result || 'Erro ao criar sistema'));
            btnSubmit.disabled = false;
            btnSubmit.innerHTML = btnOriginal;
        }
    } catch (error) {
        console.error('Erro:', error);
        mostrarErro('Erro ao criar sistema: ' + error.message);
        btnSubmit.disabled = false;
        btnSubmit.innerHTML = btnOriginal;
    }
});

function copiarChaveSweetAlert() {
    const input = document.getElementById('chaveApi');
    navigator.clipboard.writeText(input.value).then(() => {
        const btn = document.querySelector('#chaveApi + button');
        ...
        mostrarErro('Erro ao copiar a chave de API.');
    });
}
</script>
