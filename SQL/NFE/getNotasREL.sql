select
    ...
    n.modelo,
    (
        SELECT pa.idtipopagamento
        FROM pagamentos pa
        WHERE
            pa.idempresa = p.idempresa
    ...
    (
        ...
        FROM
            pagamentos pa
            JOIN tipo_pagamento tp ON tp.idempresa = pa.idempresa
            AND tp.idtipopagamento = pa.idtipopagamento
        WHERE
            pa.idempresa = p.idempresa
            AND pa.idpedidovenda = p.idpedidovenda
        ORDER BY pa.valor DESC, pa.idtipopagamento ASC
...
from
    pedido_venda p
    left join nota_fiscal n on n.idempresa = p.idempresa
    and n.idpedidovenda = p.idpedidovenda
where
